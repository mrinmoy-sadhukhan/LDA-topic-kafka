    ...
    def index(self, i, q):
        while True:
            ...
            for token in tokens:
                ...
                j = j + 1
            #print(document)
            # self.documentf.append(document)
            # print(self.documentf)

            logging.info('Indexer-%s: Finished indexing document %s' % (i, document["ID"]))
            # q['tfVector']=document['tfVector']
            # self.Documents['ID'] = document['ID']
            # self.Documents['tfVector']=document['tfVector']
            # self.Documents['Description']=document['Description']
            #q = document
            q.task_done()
        # print(self.documentf)
